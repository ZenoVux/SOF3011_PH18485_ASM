		...
		<table class="table table-hover">
			...
			<tbody>
				<c:forEach var="cart" items="${carts}">
					<tr>
						<td><a href="/PH18485_ASM/cart/history?id=${cart.id}">#${cart.id}</a></td>
						<td><span id="totalMoney"><fmt:setLocale value="vi_VN" />
								<fmt:formatNumber type="currency" value="${cart.totalMoney}" /></span></td>
						<td>${cart.phoneNumber}</td>
						<td>${cart.address}</td>
						<td><c:if test="${cart.status == CartStatus.COMFIRMED}">Chưa thanh toán</c:if>
							<c:if test="${cart.status == CartStatus.COMPLETED}">Đã thanh toán</c:if></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
