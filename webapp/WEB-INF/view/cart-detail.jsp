...
<div class="row justify-content-center">
	<div class="col-md-10">
		...
		<c:if test="${cart.completedTime != null}">
			<p>
				- Thời gian thanh toán:
				<fmt:formatDate value="${cart.completedTime}"
		...
		<div class="row mt-5">
			<c:if test="${cart.status == CartStatus.COMFIRMED}">
				<div class="d-grid gap-2 col-6 m-auto mb-4">
					<button id="btnSubmit" onclick="checkOut(${cart.id})"
						class="btn btn-primary" type="button">Thanh Toán</button>
				</div>
				<div class="d-grid gap-2 col-6 m-auto mb-4">
					<button id="btnSubmit" onclick="cancel(${cart.id})"
						class="btn btn-danger" type="button">Hủy đơn hàng</button>
				</div>
			</c:if>
		</div>
	</div>
...
<script type="text/javascript">
	function checkOut(id) {
		if (confirm("Bạn muốn thanh toán đơn hàng?")) {
